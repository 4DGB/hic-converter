...
assert (genomeid in referances), "ERROR: The genome %s is not available!\n%s"%(genomeid,referror)
...
## Check input data file, bring in os
import os
...
import pandas as pd, numpy as np, gzip, subprocess


# In[5]:


## Load in the mus (Xi) data
## Set the columns we wish to use
...
data_chr = pd.DataFrame(xlines, columns = fields)
...
## Make a short file for the chromosome.
## Make a strand dictionary
...
## Set column 3 to zero. This is the fragment id and it
## must be different from the second fragment id within column 7
data_chr[3] = 0
...
assert os.path.exists(short_path), "ERROR: Unable to locate .short file on path: %s"%short_path


# In[10]:


## Format juicer command 
## Set the jarpath and java call
